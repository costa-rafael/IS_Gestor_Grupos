...
function setXlsxStatus(t){const el=document.getElementById('xlsxStatus');if(el)el.textContent=t}

function ensureXlsxLoaded(){
  if(window.XLSX){setXlsxStatus('Motor Excel: pronto');return Promise.resolve(window.XLSX);}
  if(window.__xlsxLoading)return window.__xlsxLoading;
  window.__xlsxLoading=new Promise((resolve,reject)=>{
    let i=0;
    const loadNext=()=>{
      if(i>=XLSX_CDNS.length){window.__xlsxLoading=null;setXlsxStatus('Motor Excel: indisponível');reject(new Error('Não foi possível carregar o motor de leitura Excel. Tenta novamente ou exporta para CSV.'));return;}
      const sc=document.createElement('script');
      sc.src=XLSX_CDNS[i++];
      sc.async=true;
      sc.onload=()=>window.XLSX?(setXlsxStatus('Motor Excel: pronto'),resolve(window.XLSX)):loadNext();
      sc.onerror=loadNext;
...
<div class="main">
  ...
  <div class="content">
    ...
    <div class="panel active" id="panel-1">
      ...
      <div class="tag" id="xlsxStatus" style="margin-top:10px">Motor Excel: a iniciar...</div>
      <div class="tag" id="fileStatus" style="margin-top:8px">Nenhum ficheiro selecionado</div>
...
dz.addEventListener('drop',e=>{e.preventDefault();dz.classList.remove('over');if(e.dataTransfer.files[0]){const f=e.dataTransfer.files[0];const fs=document.getElementById('fileStatus');if(fs)fs.textContent='Ficheiro: '+f.name;parseFile(f)}});
fi.addEventListener('change',e=>{if(e.target.files[0]){const f=e.target.files[0];const fs=document.getElementById('fileStatus');if(fs)fs.textContent='Ficheiro: '+f.name;parseFile(f)}});
...
function parseFile(file){
  ...
  const parse=()=>{
    const fs=document.getElementById('fileStatus');if(fs)fs.textContent='A processar: '+file.name;
    ...
    r.onload=e=>{
      try{
        ...
        const fs=document.getElementById('fileStatus');if(fs)fs.textContent='Importação concluída: '+file.name;
  ...
  setXlsxStatus('Motor Excel: a carregar...');
